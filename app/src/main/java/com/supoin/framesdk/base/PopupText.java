 ...
 */
public class PopupText extends PopupWindow {
	...
	public PopupText(Context context, int width, int height) {
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int index, long arg3) {
				...
				if (mItemOnClickListener != null) {
					mItemOnClickListener.onItemClick(mPopupItems.get(index), index);
				}
			}
		});
	}
	...
	private void populateActions() {
		...
		mListView.setAdapter(new BaseAdapter() {
			...
			public View getView(int position, View convertView, ViewGroup parent) {
				if (convertView == null) {
					convertView = LayoutInflater.from(mContext).inflate(R.layout.popup_item_list, parent, false);
				}
				TextView textView = ViewHolderUtil.get(convertView, R.id.txt_title);
				ImageView iv_pop = ViewHolderUtil.get(convertView, R.id.iv_pop);
				...
				// 设置图片
				iv_pop.setImageDrawable(item.getmDrawable());
	 ...
	 * 添加子类项
	 */
	public void addAction(PopupItem action) {
		if (action != null) {
			mPopupItems.add(action);
			mIsDirty = true;
		}
	}
