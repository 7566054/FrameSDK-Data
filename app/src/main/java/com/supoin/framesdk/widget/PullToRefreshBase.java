...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public final boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {// zc 上拉或者下拉中
			if (mIsBeingDragged) {
	...
	protected void updateUIForMode() {
		...
		if (this == getmHeaderLayout().getParent()) {
			removeView(getmHeaderLayout());
		}
		if (mMode.showHeaderLoadingLayout()) {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		setmHeaderLayout(createLoadingLayout(context, Mode.PULL_FROM_START, a));
	...
	private void pullEvent() {
		...
		if (newScrollValue != 0 && !isRefreshing()) {
			...
			case PULL_FROM_START:
			default:
				getmHeaderLayout().onPull(scale);// zc 下拉距离
				break;
			}
	...
	private final void smoothScrollTo(int newScrollValue, long duration, long delayMillis, OnSmoothScrollFinishedListener listener) {
		...
		if (oldScrollValue != newScrollValue) {
			...
			// mScrollAnimationInterpolator = new DecelerateInterpolator();
			// }
			// mCurrentSmoothScrollRunnable = new
			// SmoothScrollRunnable(oldScrollValue, newScrollValue, duration,
			// listener);
			// post(mCurrentSmoothScrollRunnable);
		}
	}
	...
	public int getHeaderScrollY() {
		return headerScrollY;
	}

	public void setHeaderScrollY(int headerScrollY) {
